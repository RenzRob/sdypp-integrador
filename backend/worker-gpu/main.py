...
logging.basicConfig(level=logging.INFO, format="%(asctime)s [WORKER-GPU] %(levelname)s %(message)s")
...
def process_fragment(ch, method, properties, body):
    try:
        ...
        result = mine_range(block_candidate, difficulty, nonce_start, nonce_end)
...
def main():
    while True:
        try:
            ...
            logger.info("GPU Worker ready, waiting for mining tasks...")
            ch.start_consuming()
        except Exception as e:
